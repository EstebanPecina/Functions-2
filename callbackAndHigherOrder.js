...
// CODE HERE 
const first = (arr,cb) => cb(arr[0]) //setting up a function and using the call back to bring up the first name in the array.
...
// CODE HERE
const last = (arr,cb) => cb(arr[arr.length - 1]) //subtracting 1 from the array
...
// CODE HERE 
const contains = (arr, name, cb) => {  //creating a function called contains and taking in 3 parameters array , name, callback.
  if(arr.includes(name) === true) {   //checking if name is included in array it will equal true
    cb(true)                          //invoking the callback to true
  } else {
      cb(false)                       //If not setting it to return back false
  }
}
...
const uniq = (array,cb) => { //setting up the function that takes in array and callback
  for(let i = 0; i < array.length; i++){  //running a loop thru array
    for(let j = i + 1; j < array.length; j++){ //
      if(array[i] === array[j]){
        array.splice(j, 1)
        j--     //removing 1 so it doesn't skip any indexes
      }
    }
  }
...
//making function with arr&callback running thru array with .foreach  
          const each = (arr, cb) => array.forEach((element, i) => cb(element, i)) 
